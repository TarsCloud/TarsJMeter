package com.tencent.tars.protocol.tars;

import com.tencent.tars.utils.HexUtil;
...
public final class TarsInputStream {
    ...
    /*public boolean skipToTag(int tag) {
        try {
            ...
            while (true) {
                ...
                if (hd.type == TarsStructBase.STRUCT_END || tag < hd.tag) {
                    return false;
                }
                if (tag == hd.tag) {
                    return true;
    ...
    public boolean skipToTag(int tag) {
        try {
            ...
            while (true) {
                int len = peakHead(hd);
                // bugfix:全是optional且为默认值时，解析错误
//				if (tag <= hd.tag || hd.type == TarsStructBase.STRUCT_END)
//					return tag == hd.tag;
                // ->
                if (hd.type == TarsStructBase.STRUCT_END || tag < hd.tag)
                    return false;
